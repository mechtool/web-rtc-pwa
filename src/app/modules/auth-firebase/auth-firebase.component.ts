...
export class AuthFirebaseComponent implements OnDestroy{
    ...
    constructor(private authService : AuthFirebaseService, public changeRef : ChangeDetectorRef, public codeProcessor : ProcessErrorCodeService, private communicateService : CommunicationService) {
	...
	that.authService.emailPasswordProvider.createEnterUser(option).then((resp : {user : any}) => {
	              that.resultUiMessage =  {code : 'Пользователь приложения с email '+ resp.user.email + 'удачно создан!'} ;
	              //Пользователь удачно создан, переход на главную страницу
		}).catch(err => {
		    that.codeProcessor.processFromEmailPass(err).then(resp =>   {
		        that.resultUiMessage = resp;
